...
classical_data = data.where(data["genre"] == "Classical").dropna().drop(columns=["genre","mode","explicit","key","time_signature"]).reset_index(drop=True) #blue
jazz_data = data.where(data["genre"] == "Jazz").dropna().drop(columns=["genre","mode","explicit","key","time_signature"]).reset_index(drop=True) #yellow
techno_data = data.where(data["genre"] == "Techno").dropna().drop(columns=["genre","mode","explicit","key","time_signature"]).reset_index(drop=True) #nyan
rock_data = data.where(data["genre"] == "Rock").dropna().drop(columns=["genre","mode","explicit","key","time_signature"]).reset_index(drop=True) #red
